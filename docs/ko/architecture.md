...
띄워도 가용량 검사와 예약 기록이 한 번에 하나만 지나가며, 자원 확보에 실패하면 예약을
...
dcgm-exporter 가 Prometheus 로, API가 PromQL로 읽어 콘솔로 보내는 순서다.
...
만든다. Prometheus `PodMonitor`/`ServiceMonitor`, MetalLB `IPAddressPool` 이 그것이다.
