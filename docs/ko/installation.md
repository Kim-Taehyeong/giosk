...
- LoadBalancer: ServiceLB로 대체 가능 → `metallb.install: false`
...
3. 관리자 → 노드 화면에서 GPU 노드와 GPU 개수가 보이는지
   (안 보이면 GFD 라벨부터 확인 — `kubectl get nodes --show-labels | grep gpu.product`)
...
1. `billing.mode: free`, `session.expose: nodeport`, 번들 인프라는 전부 `install: false`로
   최소 구성 설치 → 세션이 뜨는 것까지 확인
2. `monitoring.install: true` + DCGM → 노드/세션 지표가 채워지는지 확인
3. HAMi → 공유 GPU 오퍼링 가용량이 소수로 잡히는지 확인
4. MetalLB + 게이트웨이 → 외부 접속 경로 정리
