// 하드 리소스 제한(크레딧 무관 절대 상한) — 계층 해석: 개인→그룹→조직→전역.
// 각 한계값은 nullable: null(생략) = 이 레벨 미지정 → 상위 폴백.
import { apiGet, apiPut } from '../client';

// 정책 화면 — 전역(설치 고정) + 상한이 설정된 범위. /console 은 호출자 스코프로 자동 필터
...
// 전역 상한(모든 사용자의 폴백) — 런타임 조정. 전역은 폴백이라 null(미지정)이 없다.
export const setGlobalLimits = (body) => apiPut('/admin/limits/global', body);

// 스코프 편집(매니저) — /console 트리. 백엔드가 (1) 대상이 내 스코프인지, (2) 상위 상한 초과 금지를 강제한다.
...
// 설정 가능한 최대치(상위 유효 상한) — 편집 폼 "최대 N" 힌트.
