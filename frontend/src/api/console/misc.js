...
// 빌링 showback. /console 은 호출자 스코프로 필터한다(platform 은 전체, org 는 자기 조직, group 은 자기 그룹).
...
// 감사 로그. /console 은 actor 가 스코프 소속인 로그만 준다(platform 은 전체).
export const getAudit = () => apiGet('/console/audit').then((d) => ({ items: d.items || [] }));

// 충전 요청(플랫폼 승인 큐. 그룹은 조직이, 조직은 플랫폼이 승인한다). 실 백엔드다.
...
// 사용자 관리. 실 백엔드다.
// 사용자 목록. 서버측에서 검색, 필터, 페이징한다(전체를 내려받지 않는다).
//  params: { q, status, group, org, page, size } 를 주면 { items, total, page, size } 가 온다
export const getUsers = (params = {}) => {
  const qs = new URLSearchParams(
    Object.entries(params).filter(([, v]) => v !== undefined && v !== null && v !== '' && v !== 'all'),
  ).toString();
  // 단일 /console 트리다. 백엔드가 호출자 스코프로 필터한다(platform 은 전체, org 는 자기 조직, group 은 자기 그룹).
  return apiGet('/console/users' + (qs ? `?${qs}` : '')).then((d) => ({
    items: d.items || [], total: d.total ?? (d.items || []).length,
  }));
};
// 사용자 360 상세. 볼륨, 세션, 데이터셋, 가입요청, 지갑, 사용량을 한 응답으로 준다.
// /console 트리라 스코프를 검증한다. platform 은 전체, 매니저는 자기 범위 사용자만 본다.
