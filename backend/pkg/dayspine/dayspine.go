...
// Layout은 스파인 키 형식(정렬 가능한 ISO 날짜)이다. 표시용 포맷은 호출부에서 만든다.
// "%m/%d" 같은 짧은 포맷을 키로 쓰면 연말(12/31 에서 01/01 로 넘어갈 때) 정렬이 뒤집힌다.
const Layout = "2006-01-02"

// Keys는 anchor(포함)로 끝나는 days개 날짜 키를 오래된 것부터 최근 순으로 만든다.
//
