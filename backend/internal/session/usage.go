...
// GPU 지표 출처는 세션의 gpu_mode 가 결정한다.
const (
...
const (
	gpuNoneCPU         = "cpu"         // CPU 전용 세션이라 GPU 가 없다
	gpuNonePhysical    = "physical"    // 물리(SSH) 임대라 Pod 이 없어 Pod 메트릭 자체가 없다
...
// Usage는 세션 1개의 실사용 지표. 측정 불가한 값은 nil 이며 0 과 구분해야 한다
...
// SessionMetric은 알림 엔진용이다. 특정 세션의 지표 1개(session_gpu/cpu/vram, %)를 평가한다.
...
// MyUsage는 호출자 본인의 running 세션 지표를 id 별 Usage 로 반환한다(세션 목록 폴링용).
...
// AdminUsage는 running 세션 전체의 실사용 지표를 id 별 Usage 로 반환한다(관제 화면).
...
func (s *Service) usageOf(ctx context.Context, rows []Session) map[string]*Usage {
	...
	dcgmNodes := map[string]string{} // 전용 GPU 세션: instanceID 별 node(세션이 놓인 노드의 GPU 로 귀속)
	...
	for i := range rows {
		...
		default: // exclusive 나 mig 는 카드를 통째로 받으므로 노드 GPU 가 곧 그 세션 사용량이다
	...
	// CPU/RAM 은 cgroup 기반이라 GPU 공유 방식과 무관하게 항상 정확하다.
...
// nodes 는 instanceID 별 node. 왜 pod 가 아니라 node 인가: HAMi 가 클러스터 전체 device-plugin 이면
// DCGM 의 pod-resources 기반 GPU 대 pod 매핑이 HAMi vGPU device ID 와 안 맞아 워크로드 pod 라벨이
// 안 붙는다(exported_pod 없음). 전용 세션은 노드 GPU 를 통째로 쓰므로 노드 GPU 지표 = 세션 지표다.
// dcgm-exporter 의 pod 을 node 로 옮기는 건 kube_pod_info 로 한다(노드 대시보드와 같은 방식).
// 노드에 GPU 가 여러 장이면 노드 평균/합이라 같은 노드의 전용 세션들엔 동일 값이 간다
//    (노드당 1 GPU 전용 배치에서 정확하다. 세션별 카드 귀속은 DCGM pod 매핑이 필요한데 HAMi 가 막는다).
func (s *Service) fillDCGM(ctx context.Context, out map[string]*Usage, nodes map[string]string) {
	...
	for iid, node := range nodes {
		...
		if v, ok := vramTot[node]; ok && u.VramTotalMB == 0 { // 전용은 vram_mb 를 안 쓰므로 카드 총량으로 채운다
			u.VramTotalMB = int(v)
		}
	}
}
...
func (s *Service) fillHAMi(ctx context.Context, out map[string]*Usage, pods []string) {
	...
	// 사용 VRAM 을 바이트에서 MB 로. vGPU 가 여러 개면 합한다.
	if v, ok := s.met.VectorByLabel(ctx,
