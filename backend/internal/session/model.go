...
type Session struct {
	...
	// 웹 접속(code-server/jupyter) 랜덤 시크릿(비밀번호·토큰). 소유자에게만 노출한다.
	WebPassword string `gorm:"column:web_password" json:"-"`

	// 제공 접속 채널(웹은 vscode/jupyter, 물리는 ssh). 이미지 channels 에서 도출하며 전송용이라 저장하지 않는다.
...
// CreateReq는 세션 생성 요청(프론트 NewSession payload).
...
type ReconfigureReq struct {
	...
	GpuType    string `json:"gpuType"`
	GpuCount   int    `json:"gpuCount"`
	ImageID    *int64 `json:"imageId"` // 선택: 모드에 맞춰 이미지 교체(CPU 이미지에서 CUDA 이미지로 등)
	Start      bool   `json:"start"`   // true = 적용 후 바로 재개
}

// Connection은 연결정보(게이트웨이 URL·토큰).
...
// AccessInfo는 게이트웨이 단기 접속 정보(토큰 포함 URL, SSH 명령). 열 때마다 새로 발급한다.
