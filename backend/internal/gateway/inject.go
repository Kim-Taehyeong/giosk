...
// primeClient는 백엔드 인증 프라임용 클라이언트(리다이렉트 미추적 — Set-Cookie 만 수확).
...
//   - vscode(code-server): POST /login {password} → code-server 가 세팅하는 세션 쿠키를 그대로 전달.
...
func (p *Proxy) primeCookies(claims *Claims) []*http.Cookie {
	...
	// code-server 가 로그인 성공 시 세팅한 쿠키를 브라우저용으로 재설정(Domain 미지정 → 현재 서브도메인 host-only).
	var out []*http.Cookie
...
//   - vscode: 쿠키 프라임(primeCookies)으로 처리 — 요청 단위 주입 없음.
