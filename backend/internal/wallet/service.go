...
// SetUserRefill은 개인 리필을 설정한다(주기는 부모 팀 상한으로 클램프).
func (s *Service) SetUserRefill(userID int64, spec RefillSpec) error {
	iv := spec.IntervalDays
	if cap := s.repo.UserRefillCap(userID, s.platDefault()); cap > 0 && iv > cap {
		iv = cap
	}
	return s.repo.SetUserRefill(userID, spec.Recurring, iv, spec.Carryover)
}

// WithPool은 할당(grant/topup) 시 조직 풀 차감 enforce 를 활성화한다.
func (s *Service) WithPool(p Pool) *Service { s.pool = p; return s }

// MyWallet은 개인 지갑 + 최근 거래를 반환한다.
func (s *Service) MyWallet(userID int64) (*MyWalletRes, error) {
	w, err := s.repo.UserWallet(userID)
	if err != nil {
		return nil, err
	}
	hist, err := s.repo.UserTransactions(userID, 500) // 페이지네이션은 프론트(클라)에서
	if err != nil {
		return nil, err
	}
	trend, err := s.repo.DailyConsumption(userID, 210)
	if err != nil {
		return nil, err
	}
	bySession, err := s.repo.SpendBySession(userID)
	if err != nil {
		return nil, err
	}
...
// Balance는 사용자 지갑 잔액을 반환한다(지갑 없음/오류=0). 세션 생성 전 잔액 검사용.
func (s *Service) Balance(userID int64) int {
	w, err := s.repo.UserWallet(userID)
	if err != nil || w == nil {
		return 0
	}
...
func (s *Service) CreditUser(userID int64, amount int, desc string, actor *int64) error {
	...
	_, err := s.repo.ApplyUserTx(userID, TxTopup, amount, desc, "", actor)
	return err
}
...
func (s *Service) AllocateUser(userID int64, amount int, super bool, desc string, actor *int64) error {
	if amount <= 0 {
		return nil
	}
	if super {
		if s.pool != nil {
			...
			if gid, ok := s.pool.GroupOfUser(userID); ok {
				if _, err := s.repo.ApplyGroupTx(gid, TxTopup, amount, actor, ""); err != nil { // 그룹 풀 자동 가산
					return err
				}
			}
		}
	} else if s.pool != nil {
		gid, ok := s.pool.GroupOfUser(userID)
		if !ok {
			return ErrNoGroup
		}
		drawn, err := s.repo.DrawGroupTx(gid, amount, actor)
		if err != nil {
			return err
		}
		if !drawn {
...
func (s *Service) Consume(userID int64, credits int, ref string) (bool, error) {
	if credits <= 0 {
		return true, nil
	}
	w, err := s.repo.UserWallet(userID)
	...
	if _, err := s.repo.ApplyUserTx(userID, TxConsume, -credits, label, ref, nil); err != nil {
		return false, err
	}
	_ = s.repo.AddMemberConsumed(userID, credits)
	return true, nil
}
