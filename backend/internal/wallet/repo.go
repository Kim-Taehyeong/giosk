...
type Repository interface {
	UserWallet(userID int64) (*UserWallet, error)
	ApplyUserTx(userID int64, txType string, amount int, desc, ref string, actor *int64) (int, error)
	SpendBySession(userID int64) ([]SessionSpend, error) // 세션별 소비 합계(원장 ref 기준)
	UserTransactions(userID int64, limit int) ([]CreditTx, error)
	DailyConsumption(userID int64, days int) ([]DailyPoint, error) // 잔디/달력 추이
	AddMemberConsumed(userID int64, credits int) error             // showback 누적
	...
	SetUserRefill(userID int64, recurring, intervalDays int, carryover bool) error
	GroupRefillCap(groupID int64, platDefault int) int
	UserRefillCap(userID int64, platDefault int) int
}
...
// AddMemberConsumed는 사용자의 멤버십 1건에 사용량을 누적한다(showback ByUser).
func (r *gormRepo) AddMemberConsumed(userID int64, credits int) error {
	return r.db.Exec(`UPDATE memberships SET consumed = consumed + ? WHERE user_id = ? ORDER BY id LIMIT 1`, credits, userID).Error
}

// UserWallet은 지갑을 보장(없으면 생성)하고 반환한다.
func (r *gormRepo) UserWallet(userID int64) (*UserWallet, error) {
	if err := r.ensureUserWallet(userID); err != nil {
		return nil, err
	}
	var w UserWallet
...
func (r *gormRepo) ensureUserWallet(userID int64) error {
	return r.db.Exec(`INSERT IGNORE INTO user_wallets (user_id) VALUES (?)`, userID).Error
}

// ApplyUserTx는 잔액을 갱신하고 거래를 기록한다(트랜잭션). 갱신 후 잔액 반환.
func (r *gormRepo) ApplyUserTx(userID int64, txType string, amount int, desc, ref string, actor *int64) (int, error) {
	var newBalance int
	err := r.db.Transaction(func(tx *gorm.DB) error {
		if err := tx.Exec(`INSERT IGNORE INTO user_wallets (user_id) VALUES (?)`, userID).Error; err != nil {
			return err
		}
		if err := tx.Exec(`UPDATE user_wallets SET balance = balance + ? WHERE user_id = ?`, amount, userID).Error; err != nil {
			return err
		}
		if err := tx.Raw(`SELECT balance FROM user_wallets WHERE user_id = ?`, userID).Scan(&newBalance).Error; err != nil {
			return err
		}
		return tx.Exec(`INSERT INTO credit_transactions (user_id, type, amount, balance_after, description, ref, actor_user_id)
			VALUES (?, ?, ?, ?, ?, ?, ?)`, userID, txType, amount, newBalance, desc, nullIfEmpty(ref), actor).Error
	})
...
func (r *gormRepo) SpendBySession(userID int64) ([]SessionSpend, error) {
	...
	err := r.db.Raw(`
		...
		FROM (
			...
			WHERE t.user_id = ? AND t.type = 'consume' AND t.amount < 0
		) k
		LEFT JOIN sessions s ON s.instance_id = k.ref
		WHERE k.ref IS NOT NULL AND k.ref <> ''
		GROUP BY k.ref, COALESCE(s.name, k.ref)
		ORDER BY credit DESC`, userID).Scan(&out).Error
	return out, err
}

func (r *gormRepo) UserTransactions(userID int64, limit int) ([]CreditTx, error) {
	var out []CreditTx
	return out, r.db.Where("user_id = ?", userID).Order("id DESC").Limit(limit).Find(&out).Error
}

// DailyConsumption은 최근 days일간 일자별 소비 합계(양수)를 오래된→최근 순으로 반환.
// 소비/정산(amount<0)만 집계하며, 원장 전체를 일자별로 GROUP BY 한다(거래 limit 무관).
func (r *gormRepo) DailyConsumption(userID int64, days int) ([]DailyPoint, error) {
	var out []DailyPoint
	err := r.db.Raw(`
		...
		ORDER BY day ASC`, userID, days).Scan(&out).Error
	return out, err
}
